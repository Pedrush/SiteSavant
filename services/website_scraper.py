"""
This script is designed for scraping websites in a respectful manner, adhering to robots.txt
policies and extracting  information from web pages. It leverages the 'BeautifulSoup' for parsing
HTML content and 'requests' for HTTP interactions.
...
- Determines scraping permissions for URLs based on the robots.txt file, ensuring compliance with
  website policies.
- Extracts internal links, text content, and metadata from web pages, focusing on semantic HTML
  elements to capture relevant information.
- Utilizes a session-based approach with requests to efficiently manage HTTP connections and
  headers, reducing overhead and improving speed.
- Supports configurable depth-based scraping, allowing users to specify how deep the scraper
  should navigate from the starting URLs.
- Implements delay between requests to respect server load and prevent unethical, aggressive
  scraping behavior.
- Saves scraped data in a structured JSON format.

Components:
- fetch_robots_txt: Fetches and parses a website's robots.txt file, determining access permissions
  for the scraper.
- can_fetch: Checks whether scraping a particular URL is allowed, using the parsed robots.txt
  rules.
- get_internal_links: Extracts all internal links from a webpage, enabling depth-based scraping.
- extract_text: Captures the main textual content from a webpage, excluding common non-content
  areas like headers and footers.
- extract_metadata: Extracts basic metadata such as the page title, meta description, and headings
  from a webpage.
- scrape_page: Combines text and metadata extraction to gather comprehensive information from
  a single webpage.
- scrape_website: Orchestrates the scraping process, managing URL queues, respecting robots.txt
  rules, and collecting data from multiple pages.
- Utility functions for reading configuration, writing JSON files, and generating timestamps
  support the scraping workflow.
...
def fetch_robots_txt(robots_url: str, user_agent: str) -> RobotFileParser:
    ...
    try:
        ...
        if response.status_code in [401, 403]:
            ...
            logging.warning(
                f"No restrictions for {robots_url}. Response code: {response.status_code}"
            )
            rp.allow_all = True
            return rp
        else:
...
def can_fetch(url: str, user_agent: str, robots_cache: dict = None) -> bool:
    ...
    Note:
    In case of an error (such as failure in fetching or parsing robots.txt), the function logs the
    error and returns False. This is to ensure the continuity of the scraping process without
    interruption while still respecting the potential restrictions of the target website.
    TODO: "http://example.com/privatepage.html" will wrongly be True when
    "User-agent: *\nDisallow: /private*/"
    """
    ...
    if rp is None:
        logging.error(
            f"An UNEXPECTED error occurred during fetching or parsing robots.txt for {robots_url}"
        )
        return False
    else:
...
def get_internal_links(base_url: str, soup: BeautifulSoup) -> Set[str]:
    ...
    normalized_base_url = urlunparse(
        base_parsed_url._replace(
            scheme=base_parsed_url.scheme.lower(), netloc=base_netloc, fragment=""
        )
    )
    ...
    for link in soup.find_all("a", href=True):
        ...
        normalized_url = urlunparse(
            parsed_joined_url._replace(
                scheme=parsed_joined_url.scheme.lower(),
                netloc=parsed_joined_url.netloc.lower(),
                fragment="",
            )
        )
...
def extract_text(soup: BeautifulSoup) -> str:
    """Extracts and returns the main textual content from a BeautifulSoup object, focusing on
    semantic HTML elements. It avoids headers, footers, menus, and other non-essential sections by
    not selecting these elements for text extraction.

    Parameters:
    soup (BeautifulSoup): A BeautifulSoup object parsed from a webpage.

    Returns:
    str: The extracted textual content as a single string, or an empty string if main content is
    not found.
    """
...
def extract_metadata(soup: BeautifulSoup) -> dict:
    ...
    It handles cases where the meta tag might not have a 'content' attribute or the title tag is
    absent.
    ...
    Returns:
    dict: A dictionary containing the extracted metadata, including the title, meta description,
    and headings.
    ...
    metadata = {"title": title, "meta_description": meta_description, "headings": headings}
    return metadata
...
def scrape_page(soup: BeautifulSoup, url: str) -> dict:
    ...
    text and metadata such as titles, headers, and other relevant information. It's designed to
    work with webpages where the relevant data is embedded in the HTML structure.
    ...
    Returns:
    - dict: A dictionary with keys 'url', 'text', 'scrape_timestamp', and other metadata extracted
      from the page. 'text' contains the main textual content, and 'scrape_timestamp' is the
      datetime when the scrape occurred. Other metadata are extracted based on the structure of
      the webpage.

    TODO: make sure the unicode characters are decoded before writing to JSON or before indexing
    in Pinecone.
    """
...
def scrape_website(
    start_urls, user_agent: str, max_depth: int = 2, request_delay: float = 1
) -> dict:
    """
    Scrapes websites starting from a list of given URLs or a single URL, respecting robots.txt
    rules, and retrieves text and metadata from each page.
    Note: The usage of a set for managing URLs to visit does not maintain the order of URLs,
    thereby not supporting ordered scraping methods like breadth-first or depth-first search.

    Parameters:
    - start_urls (str or list): The initial URL(s) to start scraping from. Can be a single URL
      or a list of URLs.
    - user_agent (str): The user agent string to be used for HTTP requests and robots.txt
      compliance.
    - request_delay (float, optional): Minimal delay in seconds between requests. Defaults to 1
      second.
    - max_depth (int, optional): The maximum depth to follow internal links for scraping.
      Defaults to 2. Examples:
      - Depth Level 0: The scraper starts at the `scraping_start_url`. No links from this page
        are followed.
      - Depth Level 1: The scraper follows links found on the `scraping_start_url` page, scraping
        the content of these linked pages.
      - Depth Level 2: In addition to level 1, the scraper also reaches links found on the
        "Depth Level 1" pages, scraping their content as well, and so on.

    Returns:
    - list: A list of dictionaries, each containing the scraped data from a single page. Each
      dictionary contains the URL, text, and metadata of the page.

    Raises:
    - requests.RequestException: If an error occurs during the HTTP request to fetch webpage
      content.
    ...
    with requests.Session() as session:
        ...
        while to_visit:
            ...
            if scraping_allowed:
                try:
                    logging.info(
                        f"Scraping {url} - depth: {depth}, visited: {len(visited)}, to_visit: {len(to_visit)}"
                    )
