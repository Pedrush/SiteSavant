...
import pytest
import faiss
from services.embeddings_deduplicator import (
    build_faiss_index,
    validate_embeddings,
    process_and_sort_duplicates,
...
def test_process_and_sort_duplicates():
    duplicate_records = [
        (
            {"url": "url1", "detokenized_chunk": "chunk1", "extra": "info"},
            {"url": "url2", "detokenized_chunk": "chunk2", "extra": "info"},
            0.1,
        )
    ]
    ...
    expected = [
        (
            {"url": "url1", "detokenized_chunk": "chunk1"},
            {"url": "url2", "detokenized_chunk": "chunk2"},
            0.1,
        )
    ]
    assert (
        sorted_duplicates == expected
    ), "The process_and_sort_duplicates function does not correctly process the input."


def test_deduplicate_embeddings_no_duplicates():
    records = [
        {"embedding_id": 1, "embedding": [0.1, 0.2]},
        {"embedding_id": 2, "embedding": [0.3, 0.4]},
    ]
...
def test_deduplicate_embeddings_with_duplicates():
    records = [
        {"embedding_id": 1, "embedding": [0.1, 0.2]},
        {"embedding_id": 2, "embedding": [0.1, 0.2]},
        {"embedding_id": 3, "embedding": [0.3, 0.4]},
    ]
    unique_records, duplicates = deduplicate_embeddings(records, True, 0.01)
    assert len(unique_records) == 2, "Expected 2 unique records after deduplication"
    assert (
        len(duplicates) == 2
    ), f"Expected 1 duplicate pair (2 duplicate records), got {len(duplicates)}"
