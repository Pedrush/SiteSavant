...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "matches": [
                    {"metadata": {"detokenized_chunk": "Text1"}},
                    {"metadata": {"detokenized_chunk": "Text2"}},
                ]
            },
            "Text1\n\nText2",
        ),
...
def mock_cohere_service(mocker):
    mock = mocker.patch(
        "services.query_handler.CohereTextProcessingService.get_embedding",
...
def test_process_query_with_valid_input(
    mock_cohere_service, mock_pinecone_init, mock_pinecone_index
):
