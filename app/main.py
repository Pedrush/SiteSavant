...
This script acts as the central orchestrator for the SiteSavant application, handling the entire
lifecycle of chatbot interactionsbased on website data. It integrates various services for web
scraping, data processing, embeddings creation and deduplication,indexing, and chatbot
interactions. The application supports custom configurations and command-line arguments to tailor
the processing and interaction flow.

Key Components:
- Web scraping to collect data from specified URLs.
- Data processing including text embeddings creation, deduplication, and indexing for efficient
  retrieval.
...
- scrape: Initiates web scraping and data preparation. Requires a URL as an argument.
  Usage: `sitesavant scrape [URL]` - Scrapes the specified URL and prepares data for chatbot
  interaction.
- chat: Activates the chatbot using previously prepared data.
  Usage: `sitesavant chat` - Starts chatbot interaction without requiring additional data
  preparation.

Environment Configuration:
The application requires certain environment variables for accessing external APIs
(COHERE_API_KEY, PINECONE_API_KEY, OPENAI_API_KEY). Ensure these are set before running the
script.
...
def parse_arguments() -> Dict[str, Any]:
    ...
    -scrape: This command fetches and prepares data from the specified website using the starting
    URL.
    ...
    parser = argparse.ArgumentParser(
        description="SiteSavant - A chatbot for any website on-demand"
    )
    subparsers = parser.add_subparsers(dest="command", required=True, help="commands")

    parser_scrape = subparsers.add_parser(
        "scrape", help="Scrape a website and initialize the chatbot on the scraped data"
    )
    parser_scrape.add_argument("url", help="The start URL for scraping", type=str)

    parser_chat = subparsers.add_parser(
        "chat", help="Initialize the chatbot on  previously scraped data"
    )
...
def handle_data_preparation(
    config: Dict[str, Any], file_paths: Dict[str, Any], timestamp: str
) -> None:
...
def process_and_respond(
    ...
    chat_response = generate_chat_response(
        query_results=query_result, **config["chatbot_interactor"]
    )
...
def main():
    """Orchestrates the setup and operation of the SiteSavant chatbot application. This is the main
    entry point for the application, handling the entire lifecycle of chatbot interactions based on
    website data.

    Steps:
    1. Initializes logging and loads environment variables.
    2. Generates a unique timestamp for tagging data and output files.
    3. Reads application configurations from 'config/parameters.yml' and from command-line
       arguments.
    4. Executes the data preparation phase, including web scraping and embeddings processing,
       unless skipped by the user.
    5. Enters a loop for chatbot interaction, processing and responding to user queries until
       termination.
    """
